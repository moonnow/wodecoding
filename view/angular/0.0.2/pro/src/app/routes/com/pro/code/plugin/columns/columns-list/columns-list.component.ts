...
import { ColumnsService } from '@com/pro/code/plugin/columns/columns.service';
...
export class ColumnsListComponent implements OnInit {
  ...
  columns: SimpleTableColumn[] = [
    ...
    { title: '数据库表编号', index: 'dtId', width: '400px' },
    {
      ...
      buttons: [
        {
          text: '编辑',
          click: (record: any, modal: any) => this.router.navigate([ '/pro/code/plugin/columns/edit/' + record['columnsId'] ])
        },
        {
          text: '删除',
          type: 'del',
        ...
        {
          text: '详情',
          click: (record: any, modal: any) => this.router.navigate([ '/pro/code/plugin/columns/detail/' + record['columnsId'] ])
        }
      ]
    }
  ];
  ...
  addColumns() {
    this.router.navigate([ '/pro/code/plugin/columns/edit' ]);
  }

  upColumns() {
    this.router.navigate([ '/pro/code/plugin/columns/edit/' + this.list[0]['columnsId'] ]);
  }
  ...
  detailColumns() {
    this.router.navigate([ '/pro/code/plugin/columns/detail/' + this.list[0]['columnsId'] ]);
  }
