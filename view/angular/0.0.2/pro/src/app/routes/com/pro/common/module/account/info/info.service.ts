...
import { InfoQuery } from '@com/pro/common/module/account/info/info-query';
...
export class InfoService {
  ...
  constructor(
    ...
    this.url = this.toolService.url + 'pro/common/module/account/info';
  }

  save(info: Info) {
    this.parameter = new Parameter();
    this.parameter.action = this.toolService.save;
    return this.http.post(this.url + this.toolService.getUrl(this.parameter), info, this.toolService.httpOptions);
  }

  batchSave(info: Info | Array<Info>) {
    this.parameter = new Parameter();
    this.parameter.action = this.toolService.batchSave;
    if (info instanceof Array) {
      return this.http.post(this.url + this.toolService.getUrl(this.parameter), info, this.toolService.httpOptions);
    } else {
      return this.http.post(this.url + this.toolService.getUrl(this.parameter), new Array(info), this.toolService.httpOptions);
    }
  }
  ...
  update(info: Info) {
    ...
    return this.http.put(this.url + this.toolService.getUrl(this.parameter), info, this.toolService.httpOptions);
  }

  batchUpdate(info: Info | Array<Info>) {
    this.parameter = new Parameter();
    this.parameter.action = this.toolService.batchUpdate;
    if (info instanceof Array) {
      return this.http.put(this.url + this.toolService.getUrl(this.parameter), info, this.toolService.httpOptions);
    } else {
      return this.http.put(this.url + this.toolService.getUrl(this.parameter), new Array(info), this.toolService.httpOptions);
    }
  }
  ...
  batchRemove(info: Info | Array<Info>) {
    ...
    if (info instanceof Array) {
      ...
      httpOptions['body'] = new Array(info);
    }
    return this.http.request(this.toolService.DELETE, this.url + this.toolService.getUrl(this.parameter), httpOptions);
  }
